...
void LED_Ctrl(LEDn_e ledno, LEDs_e sta)
{
  ...
  case LED0:
    ...
    break;
  case LED1:
    if(sta==ON)        PTA25_OUT=0; //Turn on target LED3 
    else if(sta==OFF) PTA25_OUT=1; //Turn off target LED3 
    else if(sta==RVS) GPIO_Reverse (GPIOA, 25);//Toggle on target LED3
    break;   
  case LED3:
    if(sta==ON)        PTA27_OUT=0; //Turn on target LED2 
    else if(sta==OFF) PTA27_OUT=1; //Turn off target LED2 
    else if(sta==RVS) GPIO_Reverse (GPIOA, 27);//Toggle on target LED2
    break;    
  case LED2:
  ...
  case LEDALL:
    ...
    {       
      ...
      PTA27_OUT=0; //Turn on target LED2 
