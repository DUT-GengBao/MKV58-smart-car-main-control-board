...
//      模块通道    端口          可选范围                          建议
#define UART0_RX    PTD6       //PTA1、PTA15、PTB16、PTD6 PTE21 PTB0 PTC6      PTA1不要用（与J-LINK冲突）
#define UART0_TX    PTD7       //PTA2、PTA14、PTB17、PTD7 PTE20 PTB1 PTC7     PTA2不要用（与J-LINK冲突）
...
/*********************** UART功能函数 **************************/
//初始化
extern void UART_Init      (UARTn_e uratn, uint32 baud);             //初始化uartx模块

//接收相关代码
extern  char UART_Get_Char (UARTn_e uratn);                //等待接受1个字节
extern  char UART_Query_Char (UARTn_e uratn, char *ch);                //查询接收1个字符
extern  char UART_Query_Str  (UARTn_e uratn, char *str, uint32 max_len); //查询接收字符串
extern  char UART_Query    (UARTn_e uratn);                          //查询是否接收到一个字节
//发送相关代码
extern void UART_Put_Char   (UARTn_e uratn, char ch);                 //发送1个字节
extern void UART_Put_Buff   (UARTn_e uratn, s8 *buff, uint32 len);//发送len个字节buff
extern void UART_Put_Str    (UARTn_e uratn, const uint8 *str);       //发送字符串
