...
#define MT9V034_BRIGHT 10       //1-64

#define IMAGEH  120  //行 HEIGHT 待采集摄像头图像高度行数
#define IMAGEW  188  //列 WIDTH  待采集摄像头图像宽度列数

#define LCDH    120  //OLED显示的行数
#define LCDW    188  //OLED显示的列数

#define SCL_Out     DDRC1=1      //配置输出作为SCL_Out
#define SDA_Out     DDRC0=1      //配置作为输出作为SDA_Out
#define SDA_In      DDRC0=0      //配置作为输入作为SDA_In

#define SCL_High    PTC1_OUT=1   //配置输出高电平
#define SCL_Low     PTC1_OUT=0   //配置输出低电平
#define SDA_High    PTC0_OUT=1   //配置输出高电平
#define SDA_Low     PTC0_OUT=0   //配置输出低电平
#define SDA_Data    PTC0_IN      //读取引脚上的引脚状态
...
extern u8 Image_Use[LCDH][LCDW]; //压缩后之后用于存放屏幕显示数据
extern u16 Pixle[LCDH][LCDW];              //二值化后用于OLED显示的数据
extern uint8_t Threshold;                  //OSTU大津法计算的图像阈值
extern u8  Line_Cont;          //行计数
extern u8  Field_Over_Flag;    //场标识
extern u8 GaveValue;//平均值阈值
...
void BinaryImage(u8 Image_Data[IMAGEH][IMAGEW],uint8_t ThresholdV)  ; 
