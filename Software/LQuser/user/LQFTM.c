...
void FTM_PWM_Init(FTM_Type * ftmn, FTM_CHn_e ch, u16 mod, u16 cv)
{
  /******************* 开启时钟 和 复用IO口*******************/
   ...
   case FTM3_BASE:
    ...
    /******************** 配置时钟和分频 ********************/
    FTM_SC_REG(ftmn)    = ( 0
                                  | FTM_SC_PS(8)      //分频2^FTM_SC_PS,频率为 275M/2/16
...
{
  GPIO_Init (GPIOA,8, GPO,0);//速度方向脚初始化
  GPIO_Init (GPIOA, 9, GPO,0);
  GPIO_Init (GPIOE, 27,GPO,0);
  GPIO_Init (GPIOE, 28, GPO,0);
  
   //电机频率：275M/2/(2^4)/1000=8.5kHZ,
   FTM_PWM_Init(FTM0,FTM_CH0,1000,  0);//Mot0-PTC1
   FTM_PWM_Init(FTM0,FTM_CH1,1000,  0);//Mot1-PTC2 
   FTM_PWM_Init(FTM0,FTM_CH3,1000,  0);//Mot2-PTC3
   FTM_PWM_Init(FTM0,FTM_CH4,1000,  0);//Mot3-PTC4 
